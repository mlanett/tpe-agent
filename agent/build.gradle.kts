...
dependencies {
    // Bootstrap API is an internal source-only module whose classes are shaded into the agent JAR.
    // Use compileOnly so it does not appear as a published dependency.
    compileOnly(project(":bootstrap-api"))
    
    // Tests need access to bootstrap-api classes
...
// Create a configuration for the agent JAR artifact
val agentJarConfiguration = configurations.create("agentJar") {
...
val agentJar = tasks.register<Jar>("agentJar") {
    ...
    manifest {
        ...
        attributes(
            ...
            // The agent JAR itself will be appended to bootstrap classpath
            // This makes the bootstrap-api classes visible to all classloaders
            "Boot-Class-Path" to agentFileName
        )
    }
}

// Ensure bootstrap-api classes are shaded into both the library JAR and the agent JAR
val bootstrapApiOutput = project(":bootstrap-api")
    .extensions
...
tasks.withType<Jar>().configureEach {
    from(bootstrapApiOutput)
}

// Add the agent JAR to the configuration
artifacts {
    add(agentJarConfiguration.name, agentJar)
}
